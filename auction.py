...
from greedy import greedy_eoscsp_solver,first_slot
from utils import generate_random_esop_instance


def bid(request: Request, R:Dict[int, List[Tuple[Observation,Tuple[Satellite,float]]]]) -> Optional[Tuple[float, Tuple[Observation,float]]]:
...
def try_add(M, sig_u):
    ...
    for i, (o, (s, start)) in enumerate(M):
        ...
        if (start <= new_start <= start + o.delta) or \
           (new_start <= start <= new_start + new_obs.delta):
            idx.append(i)
            reward += o.rho
    ...
    for user in p.users:
        if user.exclusive_times:
            sub_p = EOSCSP(
                ...
                users=[user],
                requests=[r for r in p.requests if r.u == user],
